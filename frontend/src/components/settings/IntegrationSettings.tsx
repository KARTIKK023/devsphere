import { useEffect, useState } from "react";

import { GithubIcon } from "@/components/icons/GithubIcon";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
} from "@/components/ui/card";

import { getData } from "@/lib/api-client";
import { getApiBaseUrl } from "@/lib/api-config";
import { useAuthStore } from "@/store/auth.store";

import type { OAuthProviders } from "@/types/auth";

const API_BASE = getApiBaseUrl();

function GoogleIcon() {
  return (
    <svg
      className="size-5"
      viewBox="0 0 24 24"
      aria-hidden="true"
    >
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1Z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.26 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.1a6.6 6.6 0 0 1 0-4.2V7.06H2.18a11 11 0 0 0 0 9.88l3.66-2.84Z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1A11 11 0 0 0 2.18 7.06l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38Z"
      />
    </svg>
  );
}

export default function IntegrationSettings() {
  const user = useAuthStore((state) => state.user);
  const providers = user?.providers;

  const [configured, setConfigured] =
    useState<OAuthProviders | null>(null);

  useEffect(() => {
    let active = true;

    getData<OAuthProviders>("/auth/oauth/providers")
      .then((data) => {
        if (active) {
          setConfigured(data);
        }
      })
      .catch(() => {
        if (active) {
          setConfigured(null);
        }
      });

    return () => {
      active = false;
    };
  }, []);

  const start = (provider: "google" | "github") => {
    window.location.href = `${API_BASE}/auth/oauth/${provider}`;
  };

  return (
    <section className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold tracking-tight">
          Integrations
        </h2>

        <p className="mt-1 text-sm text-muted-foreground">
          Connect DevSphere with the tools you use.
        </p>
      </div>

      <div className="space-y-4">
        <IntegrationCard
          icon={<GoogleIcon />}
          title="Google"
          description="Sign in and link your Google account."
          connected={providers?.google ?? false}
          available={configured?.google ?? false}
          onConnect={() => start("google")}
        />

        <IntegrationCard
          icon={<GithubIcon className="size-5" />}
          title="GitHub"
          description="Link your GitHub identity to DevSphere."
          connected={providers?.github ?? false}
          available={configured?.github ?? false}
          onConnect={() => start("github")}
        />
      </div>
    </section>
  );
}

function IntegrationCard({
  icon,
  title,
  description,
  connected,
  available,
  onConnect,
}: {
  icon: React.ReactNode;
  title: string;
  description: string;
  connected: boolean;
  available: boolean;
  onConnect: () => void;
}) {
  return (
    <Card>
      <CardContent className="flex items-center gap-4 p-6">
        <div className="flex size-10 shrink-0 items-center justify-center rounded-lg border bg-muted/40">
          {icon}
        </div>

        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="font-medium">{title}</p>

            <Badge
              variant={
                connected ? "secondary" : "outline"
              }
            >
              {connected ? "Connected" : "Not connected"}
            </Badge>
          </div>

          <p className="mt-1 text-sm text-muted-foreground">
            {description}
          </p>
        </div>

        <Button
          variant="outline"
          onClick={onConnect}
          disabled={!available}
        >
          {connected ? "Reconnect" : "Connect"}
        </Button>
      </CardContent>
    </Card>
  );
}
